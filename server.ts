...
const handler = async (request: Request): Promise<Response> => {
  ...
  if (!url.searchParams.has("__host")) {
    return new Response("Request is formatted incorrectly. Please include __host in the query string.", { status: 400 });
  }
  ...
  if (!request_headers.has("user-agent")) copyHeader("user-agent", request_headers, request.headers);
